...
export const TimelineSection: React.FC<{ id: string, title: string, subtitle: string, items: TimelineItem[] }> = ({ id, title, subtitle, items }) => (
    <RevealSection id={id} className="py-20 md:py-32">
        <div className="space-y-12 md:space-y-24">
            <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-6 text-center md:text-left">
                <h2 className="font-display text-6xl sm:text-8xl md:text-[100px] lg:text-[120px] font-black tracking-tighter leading-none opacity-10 text-slate-900 dark:text-white uppercase">{title}</h2>
                <p className="font-black text-brand-500 uppercase tracking-[0.3em] md:tracking-[0.5em] text-[10px] pb-4">{subtitle}</p>
            </div>
                ...
                {items.map((item, i) => (
                    <div key={i} className="glass-card p-6 md:p-12 lg:p-16 rounded-3xl md:rounded-[4rem] group hover:border-brand-500/50 transition-all duration-700 shadow-2xl relative overflow-hidden flex flex-col lg:grid lg:grid-cols-12 gap-10 lg:gap-16">
                        <div className="lg:col-span-4 space-y-4 md:space-y-6">
                            <h3 className="text-3xl md:text-5xl font-black tracking-tighter group-hover:text-brand-600 transition-colors leading-none text-slate-900 dark:text-white">{item.title}</h3>
                            {item.link && (
                                <a href={item.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 text-brand-600 font-black text-lg md:text-xl tracking-tight hover:underline">
